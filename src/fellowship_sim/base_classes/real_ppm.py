...
class RealPPM:
    ...
    def __post_init__(self) -> None:
        time_since_last_fight = self.owner.state.information.delay_since_last_fight
        current_time = self.owner.state.time
    ...
    def interval_since_last_proc(self) -> float:
        ...
        if self.last_attempt_time is None:
            return 0.0
        return self.owner.state.time - self.last_attempt_time
    ...
    def check(self) -> bool:
        ...
        has_procced = False
        if prob > 0 and self.owner.state.rng.random() < prob:
            has_procced = True

        self.last_attempt_time = self.owner.state.time
