...
from fellowship_sim.base_classes import Effect, Enemy, Entity, State
...
class TestEventBusOwnerUnsubscribe:
    def test_event_bus_owner_unsubscribe(self, state_always_procs__st: State) -> None:
        bus = EventBus()
        entity = Entity(state=state_always_procs__st)
        ...
        bus.emit(EffectApplied(effect=effect, target=entity))
...
class TestComputeFinalStats:
    def test_compute_final_stats_collects_modifiers_via_bus(
        ...
        state_always_procs__st.bus.subscribe(ComputeFinalStats, handler)
    ...
    def test_compute_final_stats_collects_score_modifiers_via_bus(
        ...
        player = Elarion(state=state_always_procs__st, raw_stats=raw)
...
class TestEventTimeTagging:
    ...
    def test_effect_applied_captures_time(self, state_always_procs__st: State) -> None:
        state_always_procs__st.time = 5.0
        entity = Entity(state=state_always_procs__st)
        effect = Effect(owner=entity)
        event = EffectApplied(effect=effect, target=entity)
        assert event.time == pytest.approx(5.0)

    def test_effect_applied_different_times(self, state_always_procs__st: State) -> None:
        entity = Entity(state=state_always_procs__st)
        effect = Effect(owner=entity)
        state_always_procs__st.time = 3.0
        e1 = EffectApplied(effect=effect, target=entity)
        state_always_procs__st.time = 12.5
        e2 = EffectApplied(effect=effect, target=entity)
        assert e1.time == pytest.approx(3.0)
        assert e2.time == pytest.approx(12.5)

    def test_ability_damage_captures_time(self, state_always_procs__st: State) -> None:
        state_always_procs__st.time = 7.25
        entity = Entity(state=state_always_procs__st)
        elarion = Elarion(state=state_always_procs__st, raw_stats=RawStatsFromPercents(main_stat=1000))
        event = AbilityDamage(
    ...
    def test_ability_cast_success_captures_time(self, state_always_procs__st: State) -> None:
        state_always_procs__st.time = 2.0
        entity = Entity(state=state_always_procs__st)
        elarion = Elarion(state=state_always_procs__st, raw_stats=RawStatsFromPercents(main_stat=1000))
